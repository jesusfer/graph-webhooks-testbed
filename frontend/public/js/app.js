...
var StringUtils = class {
  ...
  static queryStringToObject(query) {
    ...
    const decode = (s2) => decodeURIComponent(s2.replace(/\+/g, " "));
    params.forEach((pair) => {
      if (pair.trim()) {
...
var CacheManager = class {
  ...
  getAccessToken(account, request, tokenKeys, targetRealm) {
    ...
    accessTokenKeys.forEach((key) => {
      if (this.accessTokenKeyMatchesFilter(key, accessTokenFilter, true)) {
        const accessToken2 = this.getAccessTokenCredential(key, correlationId);
        if (accessToken2 && this.credentialMatchesFilter(accessToken2, accessTokenFilter, correlationId)) {
    ...
    if (numAccessTokens < 1) {
      ...
      accessTokens.forEach((accessToken2) => {
        this.removeAccessToken(this.generateCredentialKey(accessToken2), correlationId);
      });
  ...
  getAccessTokensByFilter(filter, correlationId) {
    ...
    tokenKeys.accessToken.forEach((key) => {
      ...
      const accessToken2 = this.getAccessTokenCredential(key, correlationId);
      if (accessToken2 && this.credentialMatchesFilter(accessToken2, filter, correlationId)) {
        accessTokens.push(accessToken2);
      }
    });
    return accessTokens;
...
var invoke = (callback, eventName, logger, telemetryClient, correlationId) => {
  return (...args) => {
    ...
    try {
      ...
      try {
        logger.trace(JSON.stringify(e2), correlationId);
      } catch (e3) {
...
var PopTokenGenerator = class {
   ...
   * @returns
   */
  async signPopToken(accessToken2, keyId, request) {
    return this.signPayload(accessToken2, keyId, request);
  }
  /**
...
function createAccessTokenEntity(homeAccountId, environment, accessToken2, clientId, tenantId, scopes, expiresOn, extExpiresOn, base64Decode2, refreshOn, tokenType, userAssertionHash, keyId) {
  const atEntity = {
    homeAccountId,
  ...
  if (atEntity.tokenType?.toLowerCase() !== AuthenticationScheme.BEARER.toLowerCase()) {
    ...
    switch (atEntity.tokenType) {
      case AuthenticationScheme.POP:
        const tokenClaims = extractTokenClaims(accessToken2, base64Decode2);
  ...
  static async generateAuthenticationResult(cryptoObj, authority, cacheRecord, fromTokenCache, request, performanceClient, idTokenClaims, requestState, serverTokenResponse, requestId) {
    ...
    if (cacheRecord.accessToken) {
      if (cacheRecord.accessToken.tokenType === AuthenticationScheme.POP && !request.popKid) {
        ...
        accessToken2 = await popTokenGenerator.signPopToken(secret, keyId, request);
      } else {
        accessToken2 = cacheRecord.accessToken.secret;
      }
  ...
  async acquireTokenByRefreshToken(request, apiId) {
    ...
    if (isFOCI) {
      try {
        ...
        const clientMismatchErrorWithFamilyRT = e2 instanceof ServerError && e2.errorCode === INVALID_GRANT_ERROR && e2.subError === CLIENT_MISMATCH_ERROR;
...
function createNewGuid() {
  ...
  for (let i2 = 0; i2 < bytes.length; i2++) {
    text += UUID_CHARS.charAt(bytes[i2] >>> 4);
    text += UUID_CHARS.charAt(bytes[i2] & 15);
  ...
  async getItem(key, correlationId) {
    ...
    if (!item) {
      try {
        ...
        this.handleDatabaseAccessError(e2, correlationId);
      }
    }
    return item;
  }
...
var CookieStorage = class {
  ...
  getItem(key) {
    ...
    for (let i2 = 0; i2 < cookieList.length; i2++) {
      const cookie = cookieList[i2];
...
var BrowserCacheManager = class extends CacheManager {
  ...
  async migrateExistingCache(correlationId) {
    ...
    for (let i2 = 0; i2 < ACCOUNT_SCHEMA_VERSION; i2++) {
      const credentialSchema = i2;
      await this.removeStaleAccounts(i2, credentialSchema, correlationId);
    }
    for (let i2 = 0; i2 < CREDENTIAL_SCHEMA_VERSION; i2++) {
      const accountSchema = i2;
      await this.migrateIdTokens(i2, accountSchema, correlationId);
    }
    const kmsiMap = this.getKMSIValues();
    for (let i2 = 0; i2 < CREDENTIAL_SCHEMA_VERSION; i2++) {
      await this.migrateAccessTokens(i2, kmsiMap, correlationId);
      await this.migrateRefreshTokens(i2, kmsiMap, correlationId);
    }
      ...
      try {
        ...
        if (i2 > 0) {
          for (let schemaVersion = 0; schemaVersion <= CREDENTIAL_SCHEMA_VERSION; schemaVersion++) {
            ...
            const endIndex = i2 > startIndex + tokenKeysCount[schemaVersion] ? startIndex + tokenKeysCount[schemaVersion] : i2;
            if (i2 > startIndex && tokenKeysCount[schemaVersion] > 0) {
        ...
        const cacheError = createCacheError(e2);
        if (cacheError.errorCode === CacheErrorCodes_exports.cacheQuotaExceeded && i2 < maxRetries) {
          if (!accessTokenKeys.length) {
            for (let i3 = 0; i3 <= CREDENTIAL_SCHEMA_VERSION; i3++) {
              if (key === getTokenKeysCacheKey(this.clientId, i3)) {
                const tokenKeys = JSON.parse(value).accessToken;
                accessTokenKeys.push(...tokenKeys);
                tokenKeysCount[i3] = tokenKeys.length;
              } else {
                const tokenKeys = this.getTokenKeys(i3).accessToken;
                accessTokenKeys.push(...tokenKeys);
                tokenKeysCount[i3] = tokenKeys.length;
              }
            }
          }
          if (accessTokenKeys.length <= i2) {
            throw cacheError;
          }
   ...
   * @param accessToken
   */
  async setAccessTokenCredential(accessToken2, correlationId, kmsi) {
    this.logger.trace("1pondb", correlationId);
    const accessTokenKey = this.generateCredentialKey(accessToken2);
    const timestamp = Date.now().toString();
    accessToken2.lastUpdatedAt = timestamp;
    await this.setUserData(accessTokenKey, JSON.stringify(accessToken2), correlationId, timestamp, kmsi);
    const tokenKeys = this.getTokenKeys();
  ...
  async acquireToken(request, cacheLookupPolicy) {
    ...
    try {
      ...
      if (e2 instanceof NativeAuthError) {
        serverTelemetryManager.setNativeBrokerErrorCode(e2.errorCode);
      }
      throw e2;
    }
  }
...
var FetchClient = class {
  ...
  async sendGetRequestAsync(url, options) {
    ...
    try {
      ...
      throw createNetworkError(createBrowserAuthError(window.navigator.onLine ? getRequestFailed : noNetworkConnectivity2), void 0, void 0, e2);
    }
    ...
    try {
      ...
      throw createNetworkError(createBrowserAuthError(failedToParseResponse), responseStatus, responseHeaders, e2);
    }
  }
  /**
  ...
  async sendPostRequestAsync(url, options) {
    ...
    try {
      ...
      throw createNetworkError(createBrowserAuthError(window.navigator.onLine ? postRequestFailed2 : noNetworkConnectivity2), void 0, void 0, e2);
    }
    responseHeaders = getHeaderDict(response.headers);
    try {
      responseStatus = response.status;
...
function getFetchHeaders(options) {
  try {
    ...
    throw createNetworkError(createBrowserAuthError(failedToBuildHeaders), void 0, void 0, e2);
  }
}
...
var PopupClient = class extends StandardInteractionClient {
  ...
  async logoutPopupAsync(validRequest, popupParams, requestAuthority, mainWindowRedirectUri) {
    ...
    try {
      ...
      this.eventHandler.emitEvent(EventType.LOGOUT_FAILURE, this.correlationId, InteractionType.Popup, null, e2);
...
var RedirectClient = class extends StandardInteractionClient {
  ...
  async logout(logoutRequest) {
    ...
    try {
      ...
      const onRedirectNavigate = this.config.auth.onRedirectNavigate;
      if (typeof onRedirectNavigate === "function") {
        const navigate2 = onRedirectNavigate(logoutUri);
        if (navigate2 !== false) {
      ...
      this.eventHandler.emitEvent(EventType.LOGOUT_FAILURE, this.correlationId, InteractionType.Redirect, null, e2);
...
var SilentIframeClient = class extends StandardInteractionClient {
  ...
  async executeCodeFlow(request) {
    ...
    try {
      ...
      if (!authClient || !(e2 instanceof AuthError) || e2.errorCode !== BrowserConstants.INVALID_GRANT_ERROR) {
        throw e2;
      }
      this.performanceClient.addFields({
        retryError: e2.errorCode
      }, this.correlationId);
...
var SilentRefreshClient = class extends StandardInteractionClient {
  ...
  async acquireToken(request) {
    ...
    return invokeAsync(refreshTokenClient.acquireTokenByRefreshToken.bind(refreshTokenClient), RefreshTokenClientAcquireTokenByRefreshToken, this.logger, this.performanceClient, request.correlationId)(silentRequest, ApiId.acquireTokenSilent_silentFlow).catch((e2) => {
      e2.setCorrelationId(this.correlationId);
      serverTelemetryManager.cacheFailedRequest(e2);
      throw e2;
    });
...
function collectInstanceStats(currentClientId, performanceEvent, logger, correlationId) {
  ...
  const sameClientIdInstanceCount = frameInstances.filter((i2) => i2 === currentClientId).length;
...
function preflightCheck2(initialized, performanceEvent, account) {
  try {
    ...
    performanceEvent.end({ success: false }, e2, account);
    throw e2;
  }
}
var StandardController = class _StandardController {
  ...
  async initialize(request) {
    ...
    if (allowPlatformBroker) {
      try {
        ...
        this.logger.verbose(e2, correlationId);
      }
    }
  ...
  async acquireTokenRedirect(request) {
    ...
    try {
      ...
      if (atrMeasurement.event.status === 2) {
        this.performanceClient.startMeasurement(AcquireTokenRedirect, correlationId).end({ success: false }, e2, request.account);
      } else {
        atrMeasurement.end({ success: false }, e2, request.account);
      }
      this.eventHandler.emitEvent(EventType.ACQUIRE_TOKEN_FAILURE, correlationId, InteractionType.Redirect, null, e2);
      throw e2;
    }
  }
  // #endregion
  ...
  acquireTokenPopup(request) {
    ...
    return result.then((result2) => {
      ...
      this.eventHandler.emitEvent(EventType.ACQUIRE_TOKEN_FAILURE, correlationId, InteractionType.Popup, null, e2);
      atPopupMeasurement.end({
        success: false
      }, e2, request.account);
      return Promise.reject(e2);
  ...
  async ssoSilent(request) {
    ...
    return result.then((response) => {
      ...
      this.eventHandler.emitEvent(EventType.ACQUIRE_TOKEN_FAILURE, correlationId, InteractionType.Silent, null, e2);
      this.ssoSilentMeasurement?.end({
        success: false
      }, e2, request.account);
      throw e2;
  ...
  async acquireTokenSilentNoIframe(silentRequest, cacheLookupPolicy) {
    if (isPlatformAuthAllowed(this.config, this.logger, silentRequest.correlationId, this.platformAuthProvider, silentRequest.authenticationScheme) && silentRequest.account.nativeAccountId) {
      ...
      return this.acquireTokenNative(silentRequest, ApiId.acquireTokenSilent_silentFlow, silentRequest.account.nativeAccountId, cacheLookupPolicy).catch(async (e2) => {
        if (e2 instanceof NativeAuthError && isFatalNativeAuthError(e2)) {
...
var apiScope = "";
var deps;
var DEFAULT_GRAPH_SCOPES = ["User.Read"];
var EXTRA_GRAPH_SCOPES_KEY = "graph-webhooks-extra-scopes";
function initAuth(dependencies) {
  deps = dependencies;
}
function getExtraGraphScopes() {
  const stored = localStorage.getItem(EXTRA_GRAPH_SCOPES_KEY);
  if (!stored) return [];
  return stored.split(",").map((s2) => s2.trim()).filter(Boolean);
}
function saveExtraScopes(scopes) {
  localStorage.setItem(EXTRA_GRAPH_SCOPES_KEY, scopes.join(","));
}
function getAllGraphScopes() {
  const extra = getExtraGraphScopes();
  const all = [...DEFAULT_GRAPH_SCOPES, ...extra];
  const seen = /* @__PURE__ */ new Set();
  return all.filter((s2) => {
    const lower = s2.toLowerCase();
    if (seen.has(lower)) return false;
    seen.add(lower);
    return true;
  }).sort((a2, b) => a2.localeCompare(b, void 0, { sensitivity: "base" }));
}
async function initMsal(appConfig2) {
  if (!appConfig2 || !appConfig2.clientId) {
    console.warn("App config missing clientId - MSAL will not initialize.");
    return;
  }
  if (!appConfig2.apiScope) {
    console.warn("App config missing apiScope - MSAL will not initialize.");
    return;
  }
  if (appConfig2.apiScope) {
    apiScope = appConfig2.apiScope;
  }
  const msalConfig = {
    auth: {
      clientId: appConfig2.clientId,
      authority: `https://login.microsoftonline.com/${appConfig2.tenantId}`,
      redirectUri: appConfig2.redirectUri
    },
    cache: {
...
async function acquireApiTokenSilent() {
...
function setupAuthEventHandlers() {
  ...
  document.getElementById("btn-scopes-consent").addEventListener("click", async () => {
    ...
    const newScopes = raw.split(",").map((s2) => s2.trim()).filter(Boolean);
    const existing = getExtraGraphScopes();
    const merged = [...existing, ...newScopes];
    const unique = [...new Set(merged.map((s2) => s2.trim()))];
    scopesModal.hidden = true;
    if (msalInstance && currentAccount) {
      try {
        const allScopes = [.../* @__PURE__ */ new Set([...DEFAULT_GRAPH_SCOPES, ...unique])];
  ...
  scopesModal.addEventListener("click", (e2) => {
    if (e2.target === scopesModal) scopesModal.hidden = true;
  });
}
...
// src/router.ts
var handler = null;
function matchRoute(pathname = location.pathname) {
...
function initRouter(routeHandler) {
  handler = routeHandler;
  window.addEventListener("popstate", () => applyRoute());
}

// src/appNotifications/appNotificationsTable.ts
async function loadAppNotifications() {
  ...
  const btnClear = document.getElementById("btn-clear-app-notifs");
  ...
  try {
    ...
    const subResourceMap = new Map(subs.map((s2) => [s2.rowKey, s2.resource]));
    if (notifs.length === 0) {
      container.innerHTML = '<div class="empty-state">No notifications received yet.</div>';
      return;
    }
    notifs.sort((a2, b) => new Date(b.receivedAt).getTime() - new Date(a2.receivedAt).getTime());
    const lifecycleCount = notifs.filter((n2) => n2.lifecycleEvent).length;
    ...
    const rows = notifs.map((n2) => {
      const received = new Date(n2.receivedAt).toLocaleString(void 0, {
        hour12: false
      });
      const resource = subResourceMap.get(n2.subscriptionId) ?? n2.subscriptionId;
      const validityIcon = n2.clientStateValid === true ? '<span title="clientState valid" style="color:var(--success);font-size:1.1rem">&#x2705;</span>' : n2.clientStateValid === false ? '<span title="clientState mismatch" style="color:var(--danger);font-size:1.1rem">&#x274C;</span>' : '<span title="clientState not checked" style="color:var(--text-secondary);font-size:1.1rem">&#x2014;</span>';
      const tokenValidityIcon = n2.validationTokensValid === true ? `<span title="${escapeHtml(n2.validationTokensSummary ?? "Tokens valid")}" style="color:var(--success);font-size:1.1rem">&#x2705;</span>` : n2.validationTokensValid === false ? `<span title="${escapeHtml(n2.validationTokensSummary ?? "Token validation failed")}" style="color:var(--danger);font-size:1.1rem">&#x274C;</span>` : '<span title="No validation tokens" style="color:var(--text-secondary);font-size:1.1rem">&#x2014;</span>';
      const lifecycleBadge = n2.lifecycleEvent ? `<span class="lifecycle-badge" title="Lifecycle event">${escapeHtml(n2.lifecycleEvent)}</span>` : "";
      const isLifecycle = !!n2.lifecycleEvent;
      const hiddenAttr = isLifecycle && !showLifecycle ? ' style="display:none"' : "";
      return `
          <tr data-sub-id="${n2.subscriptionId}"${isLifecycle ? ' data-lifecycle="true"' : ""}${hiddenAttr}>
            <td>${received}</td>
            <td title="${n2.subscriptionId}">${escapeHtml(resource)}</td>
            ...
            <td class="actions">
              <a href="#" class="app-detail-link" data-notif-id="${n2.rowKey}" style="color:var(--primary);font-weight:600;text-decoration:none">
                View Details
              </a>
            </td>
          </tr>`;
...
async function clearAllAppNotifications() {
...
function toggleAppLifecycleRows() {
  const chk = document.getElementById("chk-show-lifecycle-app-notifs");
  const container = document.getElementById("app-notifications-container");
...
function setupAppNotificationsTableEventHandlers() {
  document.getElementById("btn-refresh-app-notifs").addEventListener("click", loadAppNotifications);
  document.getElementById("btn-clear-app-notifs").addEventListener("click", clearAllAppNotifications);
  document.getElementById("chk-show-lifecycle-app-notifs").addEventListener("change", toggleAppLifecycleRows);
}
...
// src/appNotifications/appSubscriptionsTable.ts
async function loadAppSubscriptions() {
  ...
  const btnRefresh = document.getElementById("btn-refresh-app-subs");
  if (btnRefresh) btnRefresh.disabled = true;
  try {
    const res = await apiFetch("/api/app-subscriptions");
    const subs = await res.json();
    if (subs.length === 0) {
      container.innerHTML = '<div class="empty-state">No app subscriptions yet. Create one above.</div>';
      return;
    }
    ...
    const filteredSubs = showExpired ? subs : subs.filter((s2) => {
      const isExpired = new Date(s2.expirationDateTime).getTime() < Date.now();
      const isRemoved = !!s2.removedAt;
      return !isExpired && !isRemoved;
    });
    const expiredCount = subs.filter((s2) => {
      const isExpired = new Date(s2.expirationDateTime).getTime() < Date.now();
      return isExpired || !!s2.removedAt;
    }).length;
    ...
    filteredSubs.sort((a2, b) => {
      const now = Date.now();
      const aExpiry = new Date(a2.expirationDateTime).getTime();
      const bExpiry = new Date(b.expirationDateTime).getTime();
      const aIsExpired = aExpiry < now || !!a2.removedAt;
    ...
    const rows = filteredSubs.map((s2) => {
      const expiryDate = new Date(s2.expirationDateTime);
      const expiry = formatDateTime(expiryDate);
      const isExpired = expiryDate.getTime() < Date.now();
      const isRemoved = !!s2.removedAt;
      const remainingLabel = isRemoved ? '<strong style="color:var(--danger)">(removed)</strong>' : isExpired ? '<strong style="color:var(--danger)">(expired)</strong>' : `<span style="opacity:0.7">(${formatTimeRemaining(expiryDate)})</span>`;
      const lastNotif = s2.lastNotificationAt ? formatDateTime(new Date(s2.lastNotificationAt)) : "\u2014";
      const renewBtn = isExpired || isRemoved ? "" : ` <button class="btn-primary btn-small" data-app-renew-sub="${s2.rowKey}">Renew</button>`;
      return `
          <tr>
            <td title="${s2.rowKey}">${escapeHtml2(s2.resource)}</td>
            <td>${formatChangeTypeTags(s2.changeType)}</td>
            <td style="text-align:center" title="${s2.includeResourceData ? "Include resource data enabled" : "Include resource data disabled"}">${s2.includeResourceData ? "\u{1F512}" : ""}</td>
            <td style="white-space:nowrap">${expiry}<br/>${remainingLabel}</td>
            <td style="white-space:nowrap">${lastNotif}</td>
            <td class="actions">
              <button class="btn-danger btn-small" data-app-delete-sub="${s2.rowKey}" data-app-delete-expires="${s2.expirationDateTime}">Delete</button>${renewBtn}
            </td>
          </tr>`;
    }).join("");
    container.innerHTML = `
      ...
      btn.addEventListener("click", async (e2) => {
        ...
        try {
          ...
          if (!res2.ok) {
            ...
            console.error(`Failed to renew (${res2.status}): ${errBody}`);
            alert(`Failed to renew subscription: ${res2.status}`);
            return;
          }
          loadAppSubscriptions();
        } catch (err) {
...
function setupAppSubscriptionsTableEventHandlers(onManualRefresh) {
...
var changeTypeDefaultColor = "#8764b8";
function formatChangeTypeTags(changeType) {
  return changeType.split(",").map((ct) => ct.trim()).filter(Boolean).map((ct) => {
    const color = changeTypeColors[ct.toLowerCase()] ?? changeTypeDefaultColor;
    return `<span class="change-type-tag" style="background:${color}">${escapeHtml2(ct)}</span>`;
  }).join(" ");
}
...
// src/appNotifications/createSubscription.ts
var deps2;
function initAppCreateSubscription(dependencies) {
  deps2 = dependencies;
}
async function createAppSubscription(resource, changeType, expirationMinutes, includeResourceData) {
  hideAppCreateResult();
  ...
  try {
    ...
    resetAppCreateForm();
    deps2.onAppSubscriptionCreated?.();
  } catch (err) {
...
function setAppCreateFormBusy(busy) {
  const fieldset = document.getElementById(
    "create-app-sub-fieldset"
  );
  const spinner = document.querySelector("#btn-create-app-sub .spinner");
...
var appAutoHideTimer = null;
function showAppCreateResult(message, success) {
  if (appAutoHideTimer) {
...
function resetAppCreateForm() {
  const form = document.getElementById("create-app-subscription-form");
  if (form) form.reset();
...
function setupAppCreateSubscriptionEventHandlers() {
  ...
  document.addEventListener("click", (e2) => {
    if (!appDropdown.contains(e2.target)) {
      appMenu.hidden = true;
    }
  });
  const appCheckboxes = appMenu.querySelectorAll('input[type="checkbox"]');
  const updateAppToggleLabel = () => {
    const selected = Array.from(appCheckboxes).filter((cb) => cb.checked).map((cb) => cb.value);
  ...
  document.getElementById("create-app-subscription-form").addEventListener("submit", (e2) => {
    e2.preventDefault();
    const resource = document.getElementById("app-sub-resource").value.trim();
    ...
    const expMinutes = parseInt(
      document.getElementById("app-sub-expiration").value,
      10
    ) || 60;
    const includeResourceData = document.getElementById("app-sub-includeResourceData").checked;
...
function Header({ userName, avatarUrl, isSignedIn, onSignIn, onSignOut }) {
  return /* @__PURE__ */ u2("header", { children: [
    ...
    /* @__PURE__ */ u2("div", { id: "user-info", children: [
      avatarUrl && /* @__PURE__ */ u2("img", { class: "user-avatar", src: avatarUrl }),
      userName && /* @__PURE__ */ u2("span", { id: "user-name", children: userName }),
      !isSignedIn && /* @__PURE__ */ u2("button", { class: "btn-secondary btn-header", onClick: onSignIn, children: "Sign In" }),
      isSignedIn && /* @__PURE__ */ u2("button", { class: "btn-secondary btn-header", onClick: onSignOut, children: "Sign Out" })
    ] })
  ] });
}
...
async function graphFetch(path, init2) {
  const token = await acquireGraphTokenSilent();
  const headers = new Headers();
  headers.set("Content-Type", "application/json");
  if (init2?.headers) {
    const extra = new Headers(init2.headers);
...
// src/delegatedNotifications/createSubscription.ts
var deps3;
function initCreateSubscription(dependencies) {
  deps3 = dependencies;
}
async function createSubscription(resource, changeType, expirationMinutes, includeResourceData = false) {
  hideCreateResult();
...
async function doCreateSubscription(resource, changeType, expirationMinutes, includeResourceData) {
  const expirationDateTime = new Date(Date.now() + expirationMinutes * 60 * 1e3).toISOString();
  const appConfig2 = deps3.getAppConfig();
  const notificationUrl = appConfig2?.graphNotificationUrl || "";
  if (!notificationUrl) {
    showCreateResult(
  ...
  if (includeResourceData) {
    ...
    graphPayload.encryptionCertificate = appConfig2.encryptionCertificate;
    graphPayload.encryptionCertificateId = appConfig2.encryptionCertificateId;
  }
  try {
...
function setCreateFormBusy(busy) {
  const fieldset = document.getElementById("create-sub-fieldset");
  const spinner = document.querySelector("#btn-create-sub .spinner");
...
function showCreateResult(message, success) {
  if (autoHideTimer) {
...
function setupCreateSubscriptionEventHandlers() {
  const toggle = document.getElementById("changetype-toggle");
  const menu = document.getElementById("changetype-menu");
  ...
  updateToggleLabel();
  document.getElementById("create-subscription-form").addEventListener("submit", (e2) => {
    e2.preventDefault();
    const resource = document.getElementById("sub-resource").value.trim();
    const changeType = Array.from(
      document.querySelectorAll(
    ...
    const expMinutes = parseInt(document.getElementById("sub-expiration").value, 10) || 60;
    const includeResourceData = document.getElementById("sub-includeResourceData").checked;
    createSubscription(resource, changeType, expMinutes, includeResourceData);
  });
}

// src/delegatedNotifications/notificationsTable.ts
var deps4;
function initNotificationsTable(dependencies) {
  deps4 = dependencies;
}
async function loadNotifications() {
  ...
  try {
    const [notifsRes, subsRes] = await Promise.all([
      apiFetch(`/api/notifications?userId=${encodeURIComponent(deps4.getUserId())}`),
      apiFetch(`/api/subscriptions?userId=${encodeURIComponent(deps4.getUserId())}`)
    ]);
    const notifs = await notifsRes.json();
    const subs = await subsRes.json();
    const subResourceMap = new Map(subs.map((s2) => [s2.rowKey, s2.resource]));
    ...
    const rows = notifs.map((n2) => {
      ...
      const resource = subResourceMap.get(n2.subscriptionId) ?? n2.subscriptionId;
      const validityIcon = n2.clientStateValid === true ? '<span title="clientState valid" style="color:var(--success);font-size:1.1rem">&#x2705;</span>' : n2.clientStateValid === false ? '<span title="clientState mismatch" style="color:var(--danger);font-size:1.1rem">&#x274C;</span>' : '<span title="clientState not checked" style="color:var(--text-secondary);font-size:1.1rem">&#x2014;</span>';
      const tokenValidityIcon = n2.validationTokensValid === true ? `<span title="${escapeHtml3(n2.validationTokensSummary ?? "Tokens valid")}" style="color:var(--success);font-size:1.1rem">&#x2705;</span>` : n2.validationTokensValid === false ? `<span title="${escapeHtml3(n2.validationTokensSummary ?? "Token validation failed")}" style="color:var(--danger);font-size:1.1rem">&#x274C;</span>` : '<span title="No validation tokens" style="color:var(--text-secondary);font-size:1.1rem">&#x2014;</span>';
      const lifecycleBadge = n2.lifecycleEvent ? `<span class="lifecycle-badge" title="Lifecycle event">${escapeHtml3(n2.lifecycleEvent)}</span>` : "";
      const isLifecycle = !!n2.lifecycleEvent;
      const hiddenAttr = isLifecycle && !showLifecycle ? ' style="display:none"' : "";
      return `
          <tr data-sub-id="${n2.subscriptionId}"${isLifecycle ? ' data-lifecycle="true"' : ""}${hiddenAttr}>
            <td>${received}</td>
            <td title="${n2.subscriptionId}">${escapeHtml3(resource)}</td>
            <td style="text-align:center">${lifecycleBadge}</td>
            <td style="text-align:center">${validityIcon}</td>
            <td style="text-align:center">${tokenValidityIcon}</td>
            <td class="actions">
              <a href="#" class="detail-link" data-notif-id="${n2.rowKey}" style="color:var(--primary);font-weight:600;text-decoration:none">
                View Details
              </a>
            </td>
          </tr>`;
    ...
    if (reloadSpinner) reloadSpinner.hidden = true;
    const btnRefresh2 = document.getElementById(
      "btn-refresh-notifs"
    );
    const btnClear2 = document.getElementById("btn-clear-notifs");
    if (btnRefresh2) btnRefresh2.disabled = false;
...
async function clearAllNotifications() {
  ...
  try {
    await apiFetch(`/api/notifications?userId=${encodeURIComponent(deps4.getUserId())}`, {
      method: "DELETE"
    });
    loadNotifications();
  } catch (err) {
...
function toggleLifecycleRows() {
  const chk = document.getElementById("chk-show-lifecycle-notifs");
  const container = document.getElementById("notifications-container");
  const rows = container.querySelectorAll("tr[data-lifecycle]");
  rows.forEach((row) => {
    row.style.display = chk.checked ? "" : "none";
...
function setupNotificationsTableEventHandlers() {
...
// src/delegatedNotifications/subscriptionsTable.ts
var deps5;
function initSubscriptionsTable(dependencies) {
  deps5 = dependencies;
}
async function loadSubscriptions() {
  ...
  try {
    const res = await apiFetch(
      `/api/subscriptions?userId=${encodeURIComponent(deps5.getUserId())}`
    );
    const subs = await res.json();
    if (subs.length === 0) {
      container.innerHTML = '<div class="empty-state">No subscriptions yet. Create one above.</div>';
      return;
    }
    const showExpiredChk = document.getElementById(
    ...
    const rows = filteredSubs.map((s2) => {
      const expiryDate = new Date(s2.expirationDateTime);
      const expiry = formatDateTime2(expiryDate);
      const isExpired = expiryDate.getTime() < Date.now();
      const isRemoved = !!s2.removedAt;
      const remainingLabel = isRemoved ? '<strong style="color:var(--danger)">(removed)</strong>' : isExpired ? '<strong style="color:var(--danger)">(expired)</strong>' : `<span style="opacity:0.7">(${formatTimeRemaining2(expiryDate)})</span>`;
      const lastNotif = s2.lastNotificationAt ? formatDateTime2(new Date(s2.lastNotificationAt)) : "\u2014";
      const renewBtn = isExpired || isRemoved ? ` <button class="btn-primary btn-small" data-renew-sub="${s2.rowKey}" data-renew-resource="${escapeAttr(s2.resource)}" data-renew-changetype="${escapeAttr(s2.changeType)}" data-renew-includeresourcedata="${s2.includeResourceData ? "true" : "false"}">Renew</button>` : "";
      const reauthorizeBtn = s2.needsReauthorization && !isExpired && !isRemoved ? ` <button class="btn-warning btn-small" data-reauth-sub="${s2.rowKey}">Reauthorize</button>` : "";
      return `
          <tr data-sub-row="${s2.rowKey}">
            <td title="${s2.rowKey}">${escapeHtml4(s2.resource)}</td>
            <td>${formatChangeTypeTags2(s2.changeType)}</td>
            <td style="text-align:center" title="${s2.includeResourceData ? "Include resource data enabled" : "Include resource data disabled"}">${s2.includeResourceData ? "\u{1F512}" : ""}</td>
            <td style="white-space:nowrap">${expiry}<br/>${remainingLabel}</td>
            <td style="white-space:nowrap">${lastNotif}</td>
            <td class="actions">
              <button class="btn-danger btn-small" data-delete-sub="${s2.rowKey}" data-delete-expires="${s2.expirationDateTime}">Delete</button>${reauthorizeBtn}${renewBtn}
            </td>
          </tr>`;
    }).join("");
    container.innerHTML = `
    ...
    container.querySelectorAll("[data-delete-sub]").forEach((btn) => {
      btn.addEventListener("click", async (e2) => {
        ...
        if (confirm("Delete this subscription record?")) {
          ...
          await apiFetch(
            `/api/subscriptions/${encodeURIComponent(subId)}?userId=${encodeURIComponent(deps5.getUserId())}`,
            { method: "DELETE" }
          );
    ...
    container.querySelectorAll("[data-reauth-sub]").forEach((btn) => {
      btn.addEventListener("click", async (e2) => {
        ...
        try {
          ...
          await apiFetch(
            `/api/subscriptions/${encodeURIComponent(subId)}/reauthorize?userId=${encodeURIComponent(deps5.getUserId())}`,
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
    ...
    container.querySelectorAll("[data-sub-row]").forEach((row) => {
      row.addEventListener("click", () => {
        const subId = row.dataset.subRow;
    ...
    if (reloadSpinner) reloadSpinner.hidden = true;
    const btnRefresh2 = document.getElementById("btn-refresh-subs");
    if (btnRefresh2) btnRefresh2.disabled = false;
  }
}
...
var changeTypeColors2 = {
  created: "#107c10",
  updated: "#0078d4",
  deleted: "#d13438"
};
var changeTypeDefaultColor2 = "#8764b8";
function formatChangeTypeTags2(changeType) {
  return changeType.split(",").map((ct) => ct.trim()).filter(Boolean).map((ct) => {
    const color = changeTypeColors2[ct.toLowerCase()] ?? changeTypeDefaultColor2;
    return `<span class="change-type-tag" style="background:${color}">${escapeHtml4(ct)}</span>`;
  }).join(" ");
}
function formatTimeRemaining2(expiryDate) {
  const diffMs = expiryDate.getTime() - Date.now();
  if (diffMs <= 0) return "expired";
  const totalSeconds = Math.floor(diffMs / 1e3);
  const days = Math.floor(totalSeconds / 86400);
...
function highlightNotificationsForSubscription(subscriptionId) {
  ...
  subRows.forEach((row) => {
    const rowSubId = row.dataset.subRow;
    if (rowSubId === subscriptionId) {
...
function removeHighlightsForSubscription(subscriptionId) {
  const container = document.getElementById("notifications-container");
  container.querySelectorAll(`tr[data-sub-id="${subscriptionId}"]`).forEach((row) => {
    row.classList.remove("highlight");
  });
}
function setupSubscriptionsTableEventHandlers(onManualRefresh) {
...
// src/detailsPage.ts
var currentNotificationIsApp = false;
function escapeHtml5(str) {
  const div = document.createElement("div");
  div.textContent = str;
  return div.innerHTML;
}
async function showNotificationDetail(notificationId, getUserId2) {
  ...
  try {
    ...
    let res = await apiFetch(
      `/api/notifications/${encodeURIComponent(notificationId)}?userId=${encodeURIComponent(getUserId2())}`
    );
    if (!res.ok) {
      res = await apiFetch(
    ...
    detailMeta.innerHTML = `
      <dt>Notification ID</dt><dd>${escapeHtml5(notif.rowKey)}</dd>
      <dt>Subscription ID</dt><dd>${escapeHtml5(notif.subscriptionId)}</dd>
...
function setupDetailsPageEventHandlers() {
  document.getElementById("chk-line-wrap").addEventListener(
    "change",
    (e2) => {
  ...
  document.getElementById("back-to-main").addEventListener("click", (e2) => {
    e2.preventDefault();
    if (history.length > 1) {
      history.back();
    } else {
...
var ws = null;
var deps6;
function initWebSocket(dependencies) {
  deps6 = dependencies;
}
  ...
  ws.addEventListener("message", (event) => {
    try {
      ...
      if (msg.type === "new-notification") {
        ...
        if (notifUserId === deps6.getUserId() || notifUserId === "__app__") {
          deps6.onNewNotification();
        }
      }
    } catch {
    }
...
// src/app.ts
var appConfig = null;
var subscriptionRefreshInterval = null;
var userAvatarUrl = null;
var REFRESH_INTERVAL_MS = 6e4;
function resetRefreshSpinner() {
...
function startSubscriptionRefreshCycle() {
...
function stopSubscriptionRefreshCycle() {
  if (subscriptionRefreshInterval) {
    clearInterval(subscriptionRefreshInterval);
...
async function init() {
  try {
    ...
    const entraLink = document.getElementById("entra-portal-link");
...
function showRoute(match) {
  ...
  const toggleBtn = document.getElementById("btn-switch-section");
  const title = document.getElementById("section-toggle-title");
  const consentBtn = document.getElementById("btn-consent-scopes");
...
function setupSectionToggle() {
  const btn = document.getElementById("btn-switch-section");
  const delegatedSection = document.getElementById("delegated-section");
